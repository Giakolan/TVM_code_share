...
extern "C" void kv_cache_kernel(std::vector<const DLTensor*>& data_entry, int64_t src_eid,
  ...
  std::memcpy(out_bytes, cache_bytes, static_cast<size_t>(cache_elems * elem_bytes));
  ...
  if (old_cache->ndim == src->ndim + 1 && old_cache->ndim >= 2 &&
    ...
    for (int64_t b = 0; b < batch; ++b) {
      ...
      std::memcpy(out_bytes + dst_offset * elem_bytes, src_bytes + src_offset * elem_bytes,
  ...
  std::memcpy(out_bytes + dst_offset * elem_bytes, src_bytes,
