...
class kiwipedia_Runtime : public JSONRuntimeBase {
 ...
 private:
  ...
  AddFn add_fp_{nullptr}; // 新增 add 的指標變數
  ...
  void EnsureLibraryLoaded() {
    ...
    candidates.push_back("./libmatmul.so");
    candidates.push_back("/home/tvm/tvm/src/runtime/contrib/kiwipedia/libmatmul.so");
    ...
    for (const char* p : candidates) {
      ...
      void* sym_matmul = dlsym(so_handle_, "matmul");
      if (sym_matmul) matmul_fp_ = reinterpret_cast<MatmulFn>(sym_matmul);

      void* sym_add = dlsym(so_handle_, "add");
      if (sym_add) add_fp_ = reinterpret_cast<AddFn>(sym_add);
      
      break;
    }

    ICHECK(so_handle_ != nullptr) << "Failed to load shared library.";
    ICHECK(matmul_fp_ != nullptr) << "Failed to load symbol 'matmul'";
    ICHECK(add_fp_ != nullptr) << "Failed to load symbol 'add'";
  }
